...
class Results(QDialog, Ui_window):
    ...
    def PCPmax(self):
        ...
        vlayer = QgsVectorLayer(
            self.folder
            + "/EasyBasin/Capas_SIG/Cuenca_Hidrografica/cuenca_hidrografica_atr.shp",
            "Cuenca",
            "ogr",
        )
    ...
    def setup(self):
        ...
        #     QMessageBox.warning(None, "Aviso", "Indicar Cv.")
            ...
            not self.lineEditPmax.isModified() and not self.lineEditPmax_2.isModified()
        ):
            ...
            input_pcp = (
                "C:/EasyBasin/Rasters/PCP/"
                + str(self.comboBoxPeriodo.currentText())
                + ".tif"
            )
    ...
    def intensidad(self):
        ...
        try:
            ...
            QMessageBox.warning(
                None, "Aviso", "Indicar PRECIPITACIÓN MÁXIMA DIARIA (Pd)."
            )
            return
    ...
    def escorrentia(self):
        ...
        if P0 == "---":
            QMessageBox.warning(
                None, "Aviso", "Seleccionar CAPA SIG PARA EL UMBRAL DE ESCORRENTÍA"
            )
            return
        ...
        try:
            ...
            reply = QMessageBox.question(
                None,
                "ATENCIÓN",
                "Cuenca del Levante o Sureste Peninsular (Regiones 72/821/822) con T>25 años. Cálculo QT= Ψ*Q10^λ\n¿Quieres continuar con este método de cálculo para las cuencas pequeñas del Levante y Sureste peninsular?",
                QMessageBox.Yes,
                QMessageBox.No,
            )
        ...
        if obra == "De la carretera (puentes y ODTs)":
            ...
            obra
            == "De vías de servicio, ramales, caminos, accesos y drenaje de plataforma y márgenes"
        ):
            bPM = vBm * vFT
            self.lineEdit_b.setText(str(round(bPM, 3)))
        else:
            QMessageBox.warning(
                None, "AVISO", "Seleccionar OBRAS DE DRENAJE TRANSVERSAL."
            )
            return
    ...
    def informe(self):
        ...
        with open(informe_MR, "w") as file:
            file.write(
                "------------------------------------INFORME DE RESULTADOS MÉTODO RACIONAL NORMA 5.2-IC DE EASYBASIN------------------------------------ \n \n"
            )
            file.write(
            ...
            file.write(
                "-Precipitación diaria correspondiente a "
                + T
                + " (Pd): "
                + Pd
                + " mm \n"
            ...
            file.write(
                "-Factor reductor de la precipitación por área de la cuenca (KA): "
                + KA
                + "\n"
            )
            file.write(
                "-Intensidad media diaria de precipitación corregida correspondiente a "
                + T
                + " (Id): "
                + Id
                + " mm/h \n"
            ...
            file.write(
                "-Factor obtenido del índice de torrencialidad (Fa): " + Fa + "\n"
            )
            ...
            file.write(
                "-Coeficiente corrector del umbral de escorrentía (b): " + b + "\n"
            )
            file.write(
                "      -Valor medio en la región del coeficiente corrector del umbral de escorrentía (Bm): "
                + Bm
                + "\n"
            )
            file.write(
                "      -Desviación respecto al valor medio para el intervalo de confianza del 50% (D50): "
                + D50
                + "\n"
            )
            file.write(
                "      -Factor función del período de retorno (FT): " + FT + "\n"
            )
            ...
            file.write(
                "-Coeficiente de uniformidad en la distribución temporal de la precipitación (Kt): "
                + Kt
                + "\n"
            )
            file.write("-Ψ: " + Y + "\n")
            file.write("-λ: " + X + "\n")
            file.write(
                "-Caudal máximo anual correspondiente a "
                + T
                + " (QT): "
                + QT
                + " m3/s \n"
        ...
        reply2 = QMessageBox.question(
            None,
            "ATENCIÓN",
            "¿Te gustaría generar también un informe de datos de entrada de la cuenca completa para el modelo hidrológico HEC-HMS?",
            QMessageBox.Yes,
            QMessageBox.No,
        )
        if reply2 == QMessageBox.Yes:
            ...
            Mus_K = str(
                round(
                    0.18 * ((float(Lc) / 1000) * (float(Jc) / 100) ** -0.25) ** 0.76, 2
                )
            )
    ...
    def BloquesAlternos(self):
        ...
        with open(output_txt, "w") as file:
            file.write(
                f"Hietograma de diseño generado por el plugin de QGIS *EasyBasin* para {T}.\n\n"
            )
        ...
        plt.title(
            f"Hietograma de Diseño {T} - EasyBasin", fontweight="bold", fontsize=12
        )
    ...
    def hms(self):
        ...
        for file_name in files:
            if file_name.lower().endswith(".shp"):
                ...
                params2 = {
                    "INPUT": self.folder
                    + "/EasyBasin/HMS/Red_Drenaje/Red_drenaje_LFP.shp",
                ...
                if QgsProject.instance().mapLayersByName("Cortado"):
                    subbasin_stream = QgsProject.instance().mapLayersByName("Cortado")[
                        0
                    ]
                else:
                    subbasin_stream = QgsProject.instance().mapLayersByName("Clipped")[
                        0
                    ]
                ...
                inlet_folder = (
                    self.folder + "/EasyBasin/HMS/Subcuencas/Separadas/inlets"
                )
                ...
                outlet_folder = (
                    self.folder + "/EasyBasin/HMS/Subcuencas/Separadas/outlets"
                )
                ...
                if QgsProject.instance().mapLayersByName("Matriz de distancia"):
                    streamsD = QgsProject.instance().mapLayersByName(
                        "Matriz de distancia"
                    )[0]
                else:
                    streamsD = QgsProject.instance().mapLayersByName("Distance matrix")[
                        0
                    ]
                ...
                params6 = {"INPUT": streamsDPath, "OUTPUT": "TEMPORARY_OUTPUT"}
                ...
                if QgsProject.instance().mapLayersByName("Monoparte"):
                    ...
                    streamsDcorr = QgsProject.instance().mapLayersByName(
                        "Single parts"
                    )[0]
                ...
                params7 = {"INPUT": streamsDcorrPath, "OUTPUT": "TEMPORARY_OUTPUT"}
                processing.runAndLoadResults(
                    "native:deleteduplicategeometries", params7
                )
                ...
                try:
                    ...
                    lfpSub_folder = (
                        self.folder + "/EasyBasin/HMS/Red_Drenaje/LFP_Subcuencas"
                    )
                ...
                if caps & QgsVectorDataProvider.AddAttributes:
                    layer.dataProvider().addAttributes(
                        [QgsField("LONGITUD", QVariant.Int)]
                    )
                layer.updateFields()
                ...
                elevation_folder = (
                    self.folder + "/EasyBasin/HMS/Red_Drenaje/ALT_Subcuencas"
                )
                ...
                params9 = {
                    "INPUT": vertexPath2,
                    "RASTERCOPY": self.folder
                    + "/EasyBasin/Capas_SIG/MDT/MDT_corregido.tif",
                    "COLUMN_PREFIX": "ALT",
                ...
                params10 = {
                    "INPUT": self.folder
                    + "/EasyBasin/Capas_SIG/Red_Drenaje/Red_drenaje_cuenca.shp",
                    "FIELD": [],
                ...
                processing.runAndLoadResults(
                    "native:joinattributesbylocation", params12
                )

                # Si la capa existe, la seleccionamos:
                if QgsProject.instance().mapLayersByName("Capa unida"):
                    join_subbasin = QgsProject.instance().mapLayersByName("Capa unida")[
                        0
                    ]
                else:
                    join_subbasin = QgsProject.instance().mapLayersByName(
                        "Joined layer"
                    )[0]
                ...
                params13 = {"INPUT": clipPath, "OUTPUT": "TEMPORARY_OUTPUT"}
                    ...
                    HMS_data.append(
                        {
                            "SUB": sub,
                            "AREA": area,
                            "ALT_max_basin": alt_max_basin,
                            "ALT_min_basin": alt_min_basin,
                            "Area_Impermeable": area_impermeable,
                            "p0_500m_MITECO": po_miteco,
                            "p0_100m_CLC2000": po_CLC2000,
                            "p0_100m_CLC2018": po_CLC2018,
                            "CN_500m_MITECO": CN_miteco,
                            "CN_100m_CLC2000": CN_CLC2000,
                            "CN_100m_CLC2018": CN_CLC2018,
                            "LONGITUD_Cauce": longitud_red,
                        }
                    )
                ...
                HMS_data3.append(
                    {
                        "ALT_max_Cauce_Principal": max_altitude,
                        "ALT_min_Cauce_Principal": min_altitude,
                    }
                )
        ...
        df_HMS3 = pd.DataFrame(
            HMS_data3, columns=["ALT_max_Cauce_Principal", "ALT_min_Cauce_Principal"]
        )
        ...
        df["PENDIENTE (%)"] = np.where(
            ...
            (df["ALT_max_Cauce_Principal"] - df["ALT_min_Cauce_Principal"])
            / df["LONGITUD_Cauce_Principal"]
            * 100,
        ...
        df["TIEMPO DE CONCENTRACION (HR)"] = np.where(
            df["LONGITUD_Cauce_Principal"] > df["LONGITUD_Cauce"],
            0.3
            * ((df["LONGITUD_Cauce_Principal"] / 1000) ** (0.76))
        ...
        df_MR["C"] = np.where(
            ...
            (
                (df_MR["Pd (MM)"] * df_MR["KA"] / df_MR["Po (MM)"] - 1)
                * (df_MR["Pd (MM)"] * df_MR["KA"] / df_MR["Po (MM)"] + 23)
            )
    ...
    def open(self):
        ...
        self.pushButton_labelPath2.setStyleSheet(
            "QPushButton { color: rgb(85, 0, 127); }"
        )
